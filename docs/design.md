...
The implemented LLM entrypoints are `llm-compatibility-advice`, `llm-schema-advice`, and `llm-migration-strategy`. `llm-compatibility-advice` reads the deterministic compatibility outputs `clusters/<source_cluster_id>/inventory/schema-issues.yaml` and `clusters/<source_cluster_id>/inventory/compatibility-report.md`, redacts common secrets before sending the prompt, and writes only `clusters/<source_cluster_id>/ai/compatibility-advice.md` plus an audit JSON file. `llm-schema-advice` reads `schema/schema-diff.json`, `schema/conversion-report.md`, and generated `schema/tidb-ddl/*.sql`, then writes only `clusters/<source_cluster_id>/projects/<project_id>/ai/schema-rewrite-candidates.md` plus an audit JSON file. `llm-migration-strategy` reads cluster/project metadata, `plan/migration-plan.yaml`, and available compatibility/schema/export/import/CDC/validation artifacts, then writes only `clusters/<source_cluster_id>/projects/<project_id>/ai/migration-strategy-advice.md` plus an audit JSON file. The `ai/` directories are deliberately outside `approvals/`, `state/`, `plan/`, and `evidence/`; workers do not consume these files as execution instructions or gate decisions.
