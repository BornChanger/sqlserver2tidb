...
`worker-agent` is the delivery-oriented entrypoint for the same loop. It accepts the same holder, lease TTL, interval, max iteration, and state PR draft options, prints an agent header, and then delegates to the deterministic reconcile loop. It exists so local process managers and container runtimes can run `sqlserver2tidb worker-agent ...` without depending on the lower-level `worker-reconcile --loop` spelling.
