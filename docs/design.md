...
The implemented LLM entrypoint is `llm-compatibility-advice`. It reads the deterministic compatibility outputs `clusters/<source_cluster_id>/inventory/schema-issues.yaml` and `clusters/<source_cluster_id>/inventory/compatibility-report.md`, redacts common secrets before sending the prompt, and writes only `clusters/<source_cluster_id>/ai/compatibility-advice.md` plus an audit JSON file. The `ai/` directory is deliberately outside `approvals/`, `state/`, `plan/`, and `evidence/`; workers do not consume these files as execution instructions or gate decisions.
