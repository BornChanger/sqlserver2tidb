...
The implemented LLM entrypoints are `llm-compatibility-advice`, `llm-schema-advice`, `llm-migration-strategy`, `llm-validation-analysis`, `llm-cutover-risk`, and `llm-pr-summary`. `llm-compatibility-advice` reads the deterministic compatibility outputs `clusters/<source_cluster_id>/inventory/schema-issues.yaml` and `clusters/<source_cluster_id>/inventory/compatibility-report.md`, redacts common secrets before sending the prompt, and writes only `clusters/<source_cluster_id>/ai/compatibility-advice.md` plus an audit JSON file. `llm-schema-advice` reads `schema/schema-diff.json`, `schema/conversion-report.md`, and generated `schema/tidb-ddl/*.sql`, then writes only `clusters/<source_cluster_id>/projects/<project_id>/ai/schema-rewrite-candidates.md` plus an audit JSON file. `llm-migration-strategy` reads cluster/project metadata, `plan/migration-plan.yaml`, and available compatibility/schema/export/import/CDC/validation artifacts, then writes only `clusters/<source_cluster_id>/projects/<project_id>/ai/migration-strategy-advice.md` plus an audit JSON file. `llm-validation-analysis` reads the reviewed validation plan plus available validation state, validation report, executor validation evidence, and schema context, then writes only `ai/validation-mismatch-analysis.md` plus an audit JSON file. `llm-cutover-risk` reads the reviewed cutover runbook plus available migration state, validation state, CDC checkpoint, approvals, and evidence, then writes only `ai/cutover-risk-summary.md` plus an audit JSON file. `llm-pr-summary` reads a generated deterministic `prs/<stage>-pr.md` plus relevant project metadata and plan/schema artifacts, then writes only `ai/pr-summary.md` plus an audit JSON file. The `ai/` directories are deliberately outside `approvals/`, `state/`, `plan/`, and `evidence/`; workers do not consume these files as execution instructions or gate decisions.
