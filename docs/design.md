...
Executor evidence validation also rejects negative optional command-level data metrics. For export/import evidence these metrics are stored as `data_rows`, `data_bytes`, and optional `data_sha256` when the executor output includes complete non-negative row/byte metrics and a valid SHA256 digest line; `data_sha256` is valid only together with `data_rows` and `data_bytes`.
