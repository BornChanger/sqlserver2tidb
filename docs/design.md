...
- require every command record to include RFC3339Nano `started_at` and `completed_at`, plus non-negative `duration_ms`, reject completion timestamps earlier than start timestamps, and require optional `cdc_applied_changes`, `data_rows`, and `data_bytes` values to be non-negative;
...
Executor evidence validation also rejects negative optional command-level data metrics. For export/import evidence these metrics are stored as `data_rows` and `data_bytes` when the executor output includes a complete non-negative metric pair.
...
For export/import commands, when executor output contains `exported rows: N` plus `output bytes: N` or `imported rows: N` plus `input bytes: N`, the CLI records those values as command-level `data_rows` and `data_bytes`. If only one data metric is present or a value is negative/non-numeric, the command evidence is marked failed.
