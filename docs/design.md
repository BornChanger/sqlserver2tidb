...
Executor evidence validation rejects negative optional command-level data metrics. For export/import evidence these metrics are stored as `data_rows`, `data_bytes`, and optional `data_sha256` when the executor output includes complete non-negative row/byte metrics and a valid SHA256 digest line; `data_sha256` is valid only together with `data_rows` and `data_bytes`. Successful export evidence, successful `sql-insert` import evidence, and successful local-path or `file://` `tidb-import-into` evidence must include all three audit fields (`data_rows`, `data_bytes`, and `data_sha256`) in both `validate-repo` and executor evidence PR generation. Successful remote `s3://` / `gs://` `tidb-import-into` evidence is exempt because the executor does not inspect object storage bytes directly.
