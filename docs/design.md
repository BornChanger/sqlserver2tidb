...
It marks the CDC phase as `planned`; it does not assert that CDC is enabled, caught up, or safe for cutover. It fails fast when the approved CDC plan has no tracked tables, no reviewed key columns, or required tracked-table fields are missing.
