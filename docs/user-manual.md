...
executor evidence PR 校验会拒绝负数的 `cdc_applied_changes`、`data_rows` 和 `data_bytes`，并拒绝不符合 `sha256:<64 hex chars>` 的 `data_sha256`。当 evidence 中存在 `data_rows` / `data_bytes` / `data_sha256` 时，PR body 的命令表会展示 `Data rows`、`Data bytes` 和 `Data SHA256` 列，便于 reviewer 对照 chunk/job 级别的数据量和内容摘要。
