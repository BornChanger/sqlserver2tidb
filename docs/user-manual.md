...
- 只生成当前内置 executor 支持的计划：`--export-format csv`；默认 `--import-engine sql-insert` 支持 `file://`、`http://`、`https://` URI 前缀；显式 `--import-engine tidb-import-into` 支持 `file://`、`s3://`、`gs://` URI 前缀，并会在每个 import job 写入从 inventory 列名生成的 `fields` 列表。
