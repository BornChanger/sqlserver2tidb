...
当前 CLI 在执行 `discover-sqlserver --connection-string-env ...` 时会连接 SQL Server，并且只读取 catalog metadata。`sqlserver2tidb-executor apply-ddl --execute` 可以显式连接 TiDB，执行已 review 且不含 `TODO` 的 DDL 文件。`sqlserver2tidb-executor export --execute` 也可以显式连接 SQL Server，把一个已审批的导出 work item 写成本地 `file://` CSV、HTTP(S) CSV 或原生 `s3://` CSV；CSV 会带内部 `__sqlserver2tidb_null_bitmap` 尾列，用来保留 SQL NULL。S3 export 和 `sql-insert` S3 import 使用 AWS Signature V4，需要 `AWS_ACCESS_KEY_ID`、`AWS_SECRET_ACCESS_KEY`、`AWS_REGION` 或 `AWS_DEFAULT_REGION`，可选 `AWS_SESSION_TOKEN`，兼容端点可配置 `AWS_ENDPOINT_URL` / `AWS_S3_FORCE_PATH_STYLE`；export 签名 PutObject，请求导入时签名 GET 并使用 unsigned payload。`sqlserver2tidb-executor import --execute` 可以显式连接 TiDB；默认 `--engine sql-insert` 会把本地 `file://`、HTTP(S) 或 S3 CSV 逐行写入目标表，并在识别该内部尾列时恢复 NULL；显式 `--engine tidb-import-into` 会执行 TiDB `IMPORT INTO ... FROM FILE`，支持绝对本地路径、本地 `file://`、`s3://`、`gs://` file location，本地/file/S3 CSV 会读取 header 并把内部尾列映射成 TiDB user variable 以跳过写入，GCS 远端对象存储文件则应使用 import plan 里已 review 的 `fields` 列表。`sqlserver2tidb-executor validate-count --execute` 可以显式同时连接 SQL Server 和 TiDB，对一个源/目标对象做 `COUNT(*)` 对比；`sqlserver2tidb-executor validate-query --execute` 可以执行已 review 的源端/目标端 SQL 标量查询并比较结果，用于 `checksum`、`sampled_hash`、`bucketed_count` 和 `business_sql` 检查项。`sqlserver2tidb-executor cdc-lsn --execute` 可以显式连接 SQL Server，读取当前 CDC max LSN，并在提供源对象时读取 capture instance min LSN；`sqlserver2tidb-executor cdc --execute` 可以在显式提供 `--from-lsn` / `--to-lsn` 的情况下读取 SQL Server CDC 变更并向 TiDB 执行 upsert/delete。除此之外，它不会执行原生 GCS/Azure Blob export IO、TiDB Lightning、自动把 SQL Server max LSN 写入 GitHub plan 的 CDC 流式编排、切流、通用 row digest 或 bucketed sampled hash 策略。`discover-sqlserver --dry-run` 只输出计划，不打开数据库连接，也不写 inventory 文件。`analyze-compatibility`、`generate-schema-draft`、`generate-data-plans`、`generate-cdc-plan`、`prepare-cdc-range`、`advance-cdc-checkpoint`、`generate-validation-plan`、`generate-pr-draft`、`create-pr` 的默认 dry-run、`create-worker-state-pr` 的默认 dry-run、`create-executor-evidence-pr` 的默认 dry-run、`worker-executor` 的默认 dry-run、`sqlserver2tidb-executor` 的默认 dry-run、`compute-payload-hash`、`worker-export`、`worker-import`、`worker-cdc`、`worker-validate`、`worker-reconcile --dry-run`、`worker-reconcile --execute-next`、`worker-reconcile --loop` 和 `worker-agent` 只读取并写回或汇报 GitHub metadata 文件。`worker-reconcile --state-pr-draft`、`worker-agent --state-pr-draft` 和 `generate-executor-evidence-pr-draft` 只生成 Markdown PR body，不调用 GitHub。`create-pr --execute` 会调用本地 `gh pr create`；`create-worker-state-pr --execute` 和 `create-executor-evidence-pr --execute` 会调用本地 `git` 和 `gh`。
...
- 只生成当前内置 executor 支持的计划：`--export-format csv`；默认 `--import-engine sql-insert` 支持本地 `file://`、`http://`、`https://` 或 `s3://` URI 前缀，HTTP(S) 前缀必须包含 host，S3 前缀必须包含 bucket；`--compression gzip` 会在 export/import plan 写入 `compression: gzip` 并生成 `.csv.gz` 对象名，worker-executor 会把它透传给 executor；显式 `--import-engine tidb-import-into` 的端到端可执行计划支持本地绝对 `file://` 或 `s3://` URI 前缀，S3 前缀必须包含 bucket，并会在每个 import job 写入从 inventory 列名生成的 `fields` 列表，但当前不支持与 `--compression gzip` 组合。`gs://` 仍可作为手工/外部导出后的 `tidb-import-into` source URI，但内置 export executor 尚不生成 GCS 端到端计划。
...
它会拒绝没有 chunks、缺少必需字段、`output_uri` 不是当前 executor 支持的 `file://` / HTTP(S)，或 predicate 仍包含 `TODO` 的 export plan。
...
当前 MVP 的 `worker-import` 只在 import approval 和 payload hash 匹配后，把 `plan/import-plan.yaml` 写成 planned 状态和 evidence；如果 `tidb-import-into` import job 带有 `fields`，会原样写入 `state/import-jobs.yaml`。它会拒绝没有 jobs、缺少必需字段、import job `source_uri` 与所选 import engine 不匹配、`sql-insert` import job 带有 `fields`，或 `tidb-import-into` import job 的 `fields` 为空、重复、包含不安全 user variable；当 `source_uri` 是 `gs://` 时，缺少 `fields` 也会被拒绝；S3 可由 executor 在执行前读取 header。它不执行 TiDB Lightning 或 `IMPORT INTO`，也不连接 TiDB。
...
该命令默认扫描所有 `clusters/<source_cluster_id>/projects/<project_id>/`；生产 agent 建议加 `--source-cluster-id <cluster>`，只扫描一个上游 SQL Server 集群目录。它对 `ddl`、`export`、`import`、`cdc` 和 `validation` 分别执行 approval/hash gate，输出：
...
当前 MVP 可以只读连接 SQL Server catalog 生成 inventory，可以从 inventory 生成 TiDB DDL 草稿、全量导出/导入计划草稿、CDC 计划草稿和 validation plan 草稿，并执行 metadata-only export/import/CDC/validation worker。`worker-executor` 可以在 approval/hash gate 后生成外部执行器命令；当前 export 只接受 `format: csv`，import 接受 `engine: sql-insert` 和 `engine: tidb-import-into`。`sqlserver2tidb-executor` 当前已经可以解析这些 work item 并 dry-run 输出上下文。`apply-ddl --execute` 支持把已 review 且不含 `TODO` 的 DDL 文件执行到 TiDB。`export --execute` 支持 SQL Server 到本地 `file://` CSV、HTTP(S) CSV 或原生 `s3://` CSV 的真实导出路径，并通过内部 null bitmap 尾列保留 SQL NULL；S3 上传使用 AWS Signature V4，本地临时文件落盘后再执行 PutObject。它还不支持原生 GCS/Azure Blob 客户端或 Parquet。`import --execute` 的默认 `sql-insert` 支持本地 `file://`、HTTP(S) 或 S3 CSV 到 TiDB 的流式逐行 insert 路径，HTTP(S) URI 必须包含 host，S3 URI 必须包含 bucket 和对象路径，会识别并排除内部 null bitmap 尾列，并用 `--import-batch-size` 分批提交事务；`tidb-import-into` 会执行 TiDB `IMPORT INTO ... FROM FILE`，支持绝对本地路径、本地 `file://`、`s3://`、`gs://`，S3/GCS URI 必须包含 bucket 和对象路径，其中本地/file/S3 CSV 会读取 header 并跳过内部 null bitmap 尾列，GCS 远端对象存储 CSV 需要通过 import plan 的 `fields` 列表跳过该尾列。`worker-executor --stage validation` 可以在 validation approval/hash gate 后生成 `validate-count` 和 `validate-query` 命令，`validate-count --execute` 支持单对象 SQL Server/TiDB 行数对比，`validate-query --execute` 支持已 review 的 checksum、sampled-hash、bucketed-count 和 business-SQL scalar-query 结果对比。`generate-validation-plan` 可以显式生成 exact-numeric checksum/sample-hash 和整数 bucketed-count scalar-query 草稿。`cdc-lsn --execute` 支持显式读取 SQL Server CDC min/max LSN，`cdc --execute` 支持显式 LSN 范围的 SQL Server CDC 读取和 TiDB upsert/delete apply，`advance-cdc-checkpoint` 可以基于成功 CDC executor evidence 推进源集群 checkpoint snapshot，`prepare-cdc-range` 可以用已提交 checkpoint 和 operator 提供的 `--to-lsn` 生成下一段 CDC plan；自动把 SQL Server max LSN 写入 GitHub plan 和长周期流式编排仍是后续能力。通用行级 digest、生产级 bucketed sampled-hash 和大规模校验引擎仍是后续能力。
...
该命令读取源集群 inventory 和项目 metadata，写回项目目录下的 `plan/export-plan.yaml` 和 `plan/import-plan.yaml`。它只生成当前内置 executor 支持的 CSV 草稿。默认 `--import-engine sql-insert` 时，`--object-uri-prefix` 必须使用本地 `file://`、`http://`、`https://` 或 `s3://` 前缀，HTTP(S) 前缀必须包含 host，S3 前缀必须包含 bucket，生成的 import job 不带 `fields`；`--compression gzip` 会生成 `.csv.gz` 对象名，并在 plan 中写入 `compression: gzip`，后续 worker-executor 会透传为 executor `--compression gzip`。显式 `--import-engine tidb-import-into` 时，端到端可执行的 `--object-uri-prefix` 必须使用本地绝对 `file://` 或 `s3://` 前缀，以匹配当前内置 export executor 和 TiDB `IMPORT INTO ... FROM FILE` 的交集，S3 前缀必须包含 bucket，并会在每个 import job 中写入 `fields`，内容是 inventory 列名加上 `@sqlserver2tidb_null_bitmap`；当前 `tidb-import-into` 不支持 `--compression gzip`。`gs://` 仍可由 import executor 执行，但需要外部导出对象并手工 review import plan。该命令不连接 SQL Server 或 TiDB，不执行导出或导入。`--chunk-size-rows` 默认是 `1000000`，`--export-format` 默认是 `csv`，`--import-engine` 默认是 `sql-insert`，`--compression` 默认是 `none`。
...
该命令支持 `ddl`、`export`、`import`、`cdc` 和 `validation`。它复用对应 stage 的 approval/hash gate，只有 approval 通过、payload hash 匹配，且 DDL 的 `schema/schema-diff.json` 已经是 `reviewed`，或 export/import/CDC/validation plan 已经是 `reviewed` 或 `approved` 时才生成执行器命令。默认外部 binary 是 `sqlserver2tidb-executor`，可以通过 `--executor-binary` 覆盖。`--source-connection-string-env`、`--target-connection-string-env` 和 `--import-batch-size` 会被渲染进生成的 executor 命令，不写入 GitHub metadata；`--require-empty-target` 只会被渲染进 `sql-insert` import 命令，用于在读取 CSV 前检查目标表是否已经非空；`tidb-import-into` import job 里的可选 `fields` 会被透传成 executor 的 `--fields` 参数；S3 `tidb-import-into` job 可以不带 `fields`，executor 会在执行前 signed GET 读取 header，`sql-insert` import job 带 `fields` 会被拒绝，CDC tracked table 的 `columns` / `key_columns` / `from_lsn` / `to_lsn` 会被透传成 executor 的 `--columns` / `--key-columns` / `--from-lsn` / `--to-lsn` 参数。默认 dry-run 只打印命令；只有加 `--execute` 才会调用外部 binary，并在 executor 子命令后自动注入 `--execute`，让随仓库提供的 executor 离开 dry-run 模式。执行模式会写回 `evidence/executor-<stage>-run.json`，记录 payload hash、每条命令、输出、exit code、每条命令的开始/结束时间和耗时；可以通过 `--command-timeout <duration>` 限制每条外部 executor 命令的最长运行时间，默认 `0` 不限制；可以通过 `--command-retries <n>` 和 `--retry-backoff <duration>` 对失败命令做有界重试，默认不重试；可以通过 `--resume` 在同一 stage、同一 approved payload hash 下跳过已有成功 evidence 的命令。`--resume` 只跳过 command ID、executor args、成功 exit code、空 command error 都匹配的命令；CDC 命令还必须已有 `cdc_applied_changes`，否则会重新执行。命令级 evidence 记录最终尝试，`attempt_count` / `attempts` 会保留每次尝试的 exit code、输出、错误和耗时，便于 PR review。超时命令会被终止，写入 failed evidence，并在 command `error` 字段记录 `command timed out after <duration>`；非 `validation` stage 在重试耗尽后仍然 fail-fast，`validation` stage 会继续汇总后续命令结果。CDC executor 必须输出 `applied changes: N`，CLI 会把 N 解析成结构化的 `cdc_applied_changes` 字段，便于 PR review、checkpoint 推进和后续自动汇总。如果 CDC executor 缺少该行或输出非数字值，CLI 会先写 failed evidence，再返回非零退出码。`validation` stage 的执行模式会跑完所有 validation command，再在存在任意失败命令时写 failed evidence 并返回非零退出码；这样一个 mismatch 不会遮住后续检查结果。`ddl`、`export`、`import` 和 `cdc` stage 仍然是 fail-fast：如果某条命令失败，会先写已执行命令的 failed evidence，再返回非零退出码。`ddl` stage 会为 `schema/tidb-ddl/*.sql` 生成 `apply-ddl` 命令；审批后的 export/import/CDC plan 如果没有任何 work item，会直接失败；审批后的 CDC tracked table 如果没有 `columns`、没有 `key_columns`、key columns 不在 captured columns 中、没有 `from_lsn` 或没有 `to_lsn` 也会失败；export chunk predicate 如果仍包含 `TODO` 也会失败；export plan 只有 `format: csv` 且 export chunk `output_uri` 为当前 executor 支持的 `file://`、HTTP(S) 或 `s3://` 时才会生成当前 executor 命令，import plan 只有 `engine: sql-insert` 或 `engine: tidb-import-into`、import job `source_uri` 与 engine 匹配，且 import job `fields` 只用于 `tidb-import-into` 时才会生成当前 executor 命令；`validation` stage 会为 `row_count` 检查生成 `validate-count` 命令，为 `checksum`、`sampled_hash`、`bucketed_count` 和 `business_sql` 检查生成 `validate-query` 命令，如果审批后的 validation plan 没有任何支持的 row-count、checksum、sampled-hash、bucketed-count 或 business-SQL 检查，也会直接失败。当前随仓库提供的 `sqlserver2tidb-executor apply-ddl --execute` 可以把已 review DDL 执行到 TiDB；`export --execute` 支持 SQL Server 到本地 `file://` CSV、HTTP(S) CSV 或原生 `s3://` CSV，并写入内部 null bitmap 尾列；`import --execute --engine sql-insert` 支持本地 `file://`、HTTP(S) 或 S3 CSV 到 TiDB 的流式逐行 insert，并识别该尾列恢复 NULL，可选 `--require-empty-target` 在打开 CSV 前做非空目标表预检，HTTP(S) source URI 必须包含 host，S3 source URI 必须包含 bucket 和对象路径；`import --execute --engine tidb-import-into` 支持 TiDB `IMPORT INTO ... FROM FILE`，要求目标表已存在且为空，file location 为本地路径、`file://`、`s3://` 或 `gs://`，S3/GCS source URI 必须包含 bucket 和对象路径；本地/file/S3 可自动读取 CSV header，也可通过 `--fields` 显式传入列名/user variable 列表；GCS 需要显式 `--fields`；`validate-query --execute` 支持单行单列 SQL 标量结果对比；`cdc-lsn --execute` 支持显式读取 SQL Server CDC min/max LSN；`cdc --execute` 支持来自 reviewed CDC plan 的 `from_lsn` / `to_lsn` 范围的 SQL Server CDC 读取和 TiDB upsert/delete apply；`advance-cdc-checkpoint` 可以基于成功的 CDC executor evidence 推进 GitHub metadata 里的源集群 checkpoint；`prepare-cdc-range` 可以基于 checkpoint 派生下一段 plan 的 `from_lsn`。当前还没有自动把 SQL Server max LSN 写入 GitHub plan 或长周期 streaming loop。

export/import executor 的数据量指标有固定输出约定：export 成功时输出 `exported rows: N`、`output bytes: N` 和 `output sha256: sha256:<digest>`；`sql-insert` import 成功时输出 `imported rows: N`、`input bytes: N` 和 `input sha256: sha256:<digest>`；本地路径或 `file://` 的 `tidb-import-into` import 会在执行 `IMPORT INTO` 前预读 CSV 做审计，成功后输出同样的 `imported rows`、`input bytes` 和 `input sha256`。`worker-executor --execute` 会把完整且非负的指标对写入 command 级 `data_rows` 和 `data_bytes`，并把合法 SHA256 写入 `data_sha256`；如果只出现其中一个行数/字节数指标、值不是非负整数，或 SHA256 格式不合法，会先写 failed evidence 再返回非零退出码。

对 export、`sql-insert` import、本地路径或 `file://` 的 `tidb-import-into` import，`worker-executor --execute` 还会把“命令 exit 0 但缺少完整数据审计输出”视为失败；这样不会先写一个看似成功、随后又被 PR gate 拒绝的 evidence。`s3://` / `gs://` 的 `tidb-import-into` 仍然豁免这条强制规则，因为当前 executor 不直接读取对象存储字节。

`validate-repo` 和 executor evidence PR 生成也会拒绝缺少完整数据审计的成功 export / `sql-insert` import / 本地 `tidb-import-into` evidence；也就是说这些成功 evidence 必须同时具备 `data_rows`、`data_bytes` 和合法 `data_sha256`。
...
导出 dry-run 会校验 `--output-uri` 是否为当前 executor 支持的 `file://`、`http://`、`https://` 或 `s3://` CSV 输出，并拒绝仍包含 `TODO` 的 predicate；它不会打开 SQL Server，也不会写 CSV。
...
也可以用 `--source-connection-string-env <ENV_NAME>` 指定其他环境变量。执行模式会拒绝仍包含 `TODO` 的 predicate，并且当前只接受 `file://`、`http://`、`https://` 和 `s3://` 输出 URI。
...
`tidb-import-into` 会执行形如 `IMPORT INTO app.orders FROM '<fileLocation>' FORMAT 'csv' WITH skip_rows=1` 的 TiDB SQL。根据 TiDB 官方文档，目标表必须已经存在且为空，执行过程不支持事务回滚；详见 https://docs.pingcap.com/tidb/stable/sql-statement-import-into/ 。当前实现支持绝对本地路径、本地 `file://`、`s3://`、`gs://` file location，并拒绝相对本地路径、带远端 host 的 `file://`、缺 bucket 的 `s3://`/`gs://`，以及缺对象路径的 `s3://`/`gs://`。执行 `IMPORT INTO` 前，executor 会先对目标表执行带引用保护的 `COUNT(*)` 预检；如果目标表不存在、无查询权限或行数非 0，会在发起 `IMPORT INTO` 前失败。该预检不能替代导入期间禁止并发 DDL/DML 的操作要求。对于本地路径、`file://` 和 `s3://`，executor 会读取 CSV header，并把内部 `__sqlserver2tidb_null_bitmap` 尾列映射到 TiDB user variable 以跳过该字段；S3 读取使用 signed GET，同时会预审计行数、字节数和 SHA256。对于 `gs://`，当前不会远程读取 header，因此必须提供 reviewed import job `fields`。S3 端到端计划可由 `generate-data-plans --import-engine tidb-import-into` 生成；GCS 对象当前需要外部导出并手工 review import plan fields。worker-executor 会把字段透传为 `--fields id,name,@sqlserver2tidb_null_bitmap`。字段列表会在 GitOps 和 executor dry-run 阶段校验，拒绝空字段、重复列名、重复 user variable、GCS 远端对象存储 source 缺少 fields，以及包含空格、分号、括号等不安全字符的 user variable。
...
### 16.23 generate-executor-evidence-pr-draft / create-executor-evidence-pr
...
27. 将 `sqlserver2tidb-executor export/import` 从本地 CSV 扩展到生产对象存储格式和导入引擎，把 validation 从 exact-numeric / bucketed-count scalar-query 草稿扩展到通用行级 digest 和生产级分桶 sampled-hash 策略，并实现/审查真实 CDC 行为。
