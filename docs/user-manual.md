...
该命令检查必需目录、必需文件、`global/policies/file-schema-policy.yaml` 中的 plan schema 映射，以及已填写的 export/import/CDC/validation plan work item 的关键字段、唯一性、import/export 依赖关系、import source URI 与 export output URI 的一致性、import source URI 与所选 import engine 的兼容性，以及未清理的 `TODO` predicate。它不会连接 SQL Server 或 TiDB，也不会执行数据迁移。
