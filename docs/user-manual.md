...
worker lease 是上游 SQL Server 集群级，避免多个 worker 同时操作同一个源集群。`phase: idle` 表示空闲占位；当 phase 是 `export`、`import`、`cdc` 或 `validation` 时，lease 必须带有非空 `holder`、`lease_id`、`project_id`、`expires_at` 和 `renewed_at`，两个时间字段必须是 RFC3339，且 `expires_at` 不能早于 `renewed_at`，否则 `validate-repo` 会拒绝该仓库状态。
