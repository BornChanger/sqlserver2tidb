...
export/import executor 的数据量指标有固定输出约定：export 成功时输出 `exported rows: N` 和 `output bytes: N`，`sql-insert` import 成功时输出 `imported rows: N` 和 `input bytes: N`。`worker-executor --execute` 会把完整且非负的指标对写入 command 级 `data_rows` 和 `data_bytes`；如果只出现其中一个指标，或值不是非负整数，会先写 failed evidence 再返回非零退出码。
...
executor execute 模式的数据通道输出可以直接用于审计：`export --execute` 成功输出导出行数和写入字节数，`import --execute --engine sql-insert` 成功输出导入行数和读取字节数。`worker-executor --execute` 会把这些输出提升为结构化 evidence 字段，避免 reviewer 只能从文本日志里人工查找。
...
executor evidence PR 校验会拒绝负数的 `cdc_applied_changes`、`data_rows` 和 `data_bytes`。当 evidence 中存在 `data_rows` / `data_bytes` 时，PR body 的命令表会展示 `Data rows` 和 `Data bytes` 两列，便于 reviewer 对照 chunk/job 级别的数据量。
