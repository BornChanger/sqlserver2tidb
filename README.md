...
Successful CSV export execution prints `exported rows: N`, `output bytes: N`, and `output sha256: sha256:<digest>`. Local `file://` CSV export writes a same-directory temporary file and atomically publishes it to the target path only after the CSV writer closes successfully. HTTP(S) export aborts the request body instead of gracefully closing the stream when CSV writing fails, so executor failures do not intentionally finalize a partial upload. Successful `sql-insert` import execution prints `imported rows: N`, `input bytes: N`, and `input sha256: sha256:<digest>`; HTTP(S) import explicitly requests `Accept-Encoding: identity` so byte and digest metrics describe the stored object bytes. `worker-executor --execute` records those values as command-level `data_rows`, `data_bytes`, and `data_sha256` in `evidence/executor-<stage>-run.json`, and fails export / `sql-insert` import commands whose successful output omits the complete data audit tuple. Executor evidence PRs render these metrics for review. `validate-repo` and executor evidence PR generation require complete data audit for successful export and `sql-insert` import evidence; `tidb-import-into` evidence is exempt until that engine emits comparable data-channel metrics.
