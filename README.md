...
- GitHub PR approval sync for schema/export/import/CDC/validation/cutover PRs: after a PR is merged, approved, and green, the CLI/workflow can write the matching GitHub-file approval metadata with a freshly computed payload hash.
...
- A `cdc-health` command for long-running CDC operations that evaluates committed checkpoint freshness, lag against SQL Server max LSN, CDC retention coverage against per-table min LSNs, writes JSON metrics and JSONL history, and can send Feishu custom bot alerts.
...
The image includes `git`, `sqlserver2tidb`, and `sqlserver2tidb-executor`, and runs as a non-root `sqlserver2tidb` user. It does not include GitHub CLI; use the host CLI or extend the image when `create-pr --execute`, `sync-github-pr-approval`, `create-worker-state-pr --execute`, or `create-executor-evidence-pr --execute` must call `gh`.
...
After the GitHub PR is reviewed, green, and merged, synchronize its approval status back into the project approval file:
