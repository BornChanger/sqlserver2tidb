...
Successful CSV export execution prints `exported rows: N` and `output bytes: N`. Successful `sql-insert` import execution prints `imported rows: N` and `input bytes: N`. `worker-executor --execute` records those values as command-level `data_rows` and `data_bytes` in `evidence/executor-<stage>-run.json`, and executor evidence PRs render them for review.
