...
Compute payload hashes and run reviewed DDL/export/import/CDC actions after the matching approval files are marked approved. `worker-export` also requires `plan/export-plan.yaml` to be `reviewed` or `approved`; a draft export plan is not executable even with an approved approval file:
...
This scans cluster/project metadata and reports which `worker-executor --stage ddl`, `worker-export`, `worker-import`, `worker-cdc`, and `worker-validate` actions are ready or blocked by approval/hash checks. Export actions are blocked while `plan/export-plan.yaml` is still `draft`. It does not execute workers, acquire leases, or write state.
