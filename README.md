...
- A `cdc-orchestrator` command that repeatedly probes SQL Server CDC max LSN through the executor, prepares the next reviewed CDC range from committed checkpoints, and writes a range PR draft while preserving GitHub approval gates.
...
The orchestrator invokes `sqlserver2tidb-executor cdc-lsn --execute`, parses `max_lsn`, and then calls the same deterministic `prepare-cdc-iteration` path. When the project is caught up it can keep polling; when a new range is prepared it writes `plan/cdc-plan.yaml` and optional `prs/cdc-range-pr.md`, then stops so operators can review, approve, execute CDC, and advance checkpoints through the existing PR gates. It does not approve, merge, run CDC apply, or advance checkpoints by itself.
...
- Extend `sqlserver2tidb-executor import` with more production import controls, including richer `IMPORT INTO` options and Azure-native bulk paths if TiDB support is selected.
