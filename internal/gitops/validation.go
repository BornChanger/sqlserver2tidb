	...
	for _, check := range checks {
		...
		case "checksum", "sampled_hash", "business_sql":
			...
			if strings.TrimSpace(check.TargetSQL) == "" {
				return fmt.Errorf("%s check %s target_sql is required", check.Type, check.ID)
			}
			if containsTODOMarker(check.SourceSQL) {
				return fmt.Errorf("%s check %s source_sql still contains TODO", check.Type, check.ID)
			}
			if containsTODOMarker(check.TargetSQL) {
				return fmt.Errorf("%s check %s target_sql still contains TODO", check.Type, check.ID)
			}
		}
	}
	return nil
