...
func TestInitRepoCreatesGlobalStructure(t *testing.T) {
	...
	assertContains(t, exportSchema, `"null_encoding": {"enum": ["bitmap", "backslash-n"]}`)
	importSchema := readFile(t, root, "global/schemas/import-plan.schema.json")
	assertContains(t, importSchema, `"engine": {"enum": ["sql-insert", "tidb-import-into", "import-into", "tidb-lightning", "lightning"]}`)
	assertContains(t, importSchema, `"data_source_uri": {"type": "string"}`)
...
func TestValidateRepoReportsInvalidCDCCheckpointPhase(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, `{"status":"pending","databases":[]}`)
	rel := "clusters/prod-sqlserver-a/state/cdc-checkpoint.yaml"
	stateYAML := readFile(t, root, rel)
	stateYAML = strings.Replace(stateYAML, "checkpoints: []", "phase: export\ncheckpoints: []", 1)
	writeFileForTest(t, root, rel, stateYAML)
	...
	if report.Valid {
		t.Fatal("ValidateRepo() valid = true, want invalid CDC checkpoint phase")
	}
	assertContains(t, strings.Join(report.Errors, "\n"), `invalid cluster state clusters/prod-sqlserver-a/state/cdc-checkpoint.yaml: CDC checkpoint phase "export" does not match expected phase "cdc"`)
}
...
func TestValidateRepoReportsInvalidSchemaDiffStatus(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, `{"status":"pending","databases":[]}`)
	diffRel := "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/schema/schema-diff.json"
	writeFileForTest(t, root, diffRel, `{
  "status": "ready"
	...
	if report.Valid {
		t.Fatal("ValidateRepo() valid = true, want invalid schema diff status")
	}
...
func TestValidateRepoReportsInvalidValidationStatusPhase(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, `{"status":"pending","databases":[]}`)
	rel := "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/state/validation-status.yaml"
	stateYAML := readFile(t, root, rel)
	stateYAML = strings.Replace(stateYAML, "checks: []", "phase: export\nchecks: []", 1)
	writeFileForTest(t, root, rel, stateYAML)
	...
	if report.Valid {
		t.Fatal("ValidateRepo() valid = true, want invalid validation status phase")
	}
	assertContains(t, strings.Join(report.Errors, "\n"), `invalid state file clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/state/validation-status.yaml: validation status phase "export" does not match expected phase "validation"`)
}
...
func TestValidateRepoReportsInvalidPendingApprovalApprovedAt(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, `{"status":"pending","databases":[]}`)
	approvalRel := "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/approvals/export-approval.yaml"
	approvalYAML := readFile(t, root, approvalRel)
	...
	if report.Valid {
		t.Fatal("ValidateRepo() valid = true, want invalid pending approval approved_at")
	}
	assertContains(t, strings.Join(report.Errors, "\n"), `invalid approval clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/approvals/export-approval.yaml: approval approved_at must be RFC3339`)
}
...
func TestValidateRepoReportsMissingReviewPlanStatus(t *testing.T) {
	tests := []struct {
		name      string
		rel       string
		planKind  string
	...
	}{
		{
			...
			rel:       "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/export-plan.yaml",
			planKind:  "export plan",
			wantError: `invalid export plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/export-plan.yaml: export plan status is required`,
		},
		{
			name:      "import",
			rel:       "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml",
			planKind:  "import plan",
			wantError: `invalid import plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml: import plan status is required`,
		},
		{
			name:      "cdc",
			rel:       "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/cdc-plan.yaml",
			planKind:  "cdc plan",
			wantError: `invalid cdc plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/cdc-plan.yaml: cdc plan status is required`,
		},
		{
			name:      "validation",
			rel:       "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/validation-plan.yaml",
			planKind:  "validation plan",
			wantError: `invalid validation plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/validation-plan.yaml: validation plan status is required`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			createValidationWorkerProject(t, root, `{"status":"pending","databases":[]}`)
			planYAML := readFile(t, root, tt.rel)
			planYAML = strings.Replace(planYAML, "status: draft\n", "", 1)
			writeFileForTest(t, root, tt.rel, planYAML)
			...
			if report.Valid {
				t.Fatalf("ValidateRepo() valid = true, want missing %s status", tt.planKind)
			}
			assertContains(t, strings.Join(report.Errors, "\n"), tt.wantError)
		})
	}
...
func TestValidateRepoAcceptsS3ExportOutputURI(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	writeFileForTest(t, root, "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/export-plan.yaml", `status: reviewed
format: csv
  ...
  - source_object: sales.dbo.orders
    ...
    chunks:
      - id: dbo.orders.000001
        estimated_rows: 10
        predicate: id >= 1 and id < 11
        output_uri: ftp://migration-bucket/dbo.orders.000001.csv
`)
	...
	assertContains(t, strings.Join(report.Errors, "\n"), "invalid export plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/export-plan.yaml: export chunk dbo.orders.000001 output_uri scheme ftp is not supported by sqlserver2tidb-executor")
}

func TestValidateRepoReportsInvalidExportPlanSourceObjectName(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	writeFileForTest(t, root, "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/export-plan.yaml", `status: reviewed
format: csv
...
`)
	...
	assertContains(t, strings.Join(report.Errors, "\n"), "invalid export plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/export-plan.yaml: export chunk dbo.orders.000001 source_object must be schema.table or database.schema.table")
}
...
`)
	...
	assertContains(t, strings.Join(report.Errors, "\n"), "invalid import plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml: import engine bulk-loader is not supported by sqlserver2tidb-executor")
}
...
jobs:
  - id: import-dbo.orders.000001
    target_object: app.orders
    source_uri: s3://migration-bucket/full/dbo.orders.000001.csv
    depends_on_export_chunk: dbo.orders.000001
`)
...
func TestValidateRepoReportsInvalidImportPlanTargetObjectName(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	writeFileForTest(t, root, "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml", `status: reviewed
jobs:
...
`)
	...
	assertContains(t, strings.Join(report.Errors, "\n"), "invalid import plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml: import job import-dbo.orders.000001 target_object must be table or database.table")
}
...
func TestValidateRepoReportsInvalidCDCPlanTargetObjectName(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	writeFileForTest(t, root, "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/cdc-plan.yaml", `status: reviewed
mode: sqlserver-cdc
...
`)
	...
	assertContains(t, strings.Join(report.Errors, "\n"), "invalid cdc plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/cdc-plan.yaml: cdc tracked table sales.dbo.orders target_object must be table or database.table")
}
...
func TestValidateRepoReportsInvalidValidationPlanTargetObjectName(t *testing.T) {
	root := t.TempDir()
	must(t, InitRepo(root))
	must(t, CreateCluster(root, ClusterSpec{
		ClusterID:              "prod-sqlserver-a",
...
`)
	...
	assertContains(t, strings.Join(report.Errors, "\n"), "invalid validation plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/validation-plan.yaml: row_count check orders-row-count target_object must be table or database.table")
}
...
`)
	...
	body := readFile(t, root, draft.BodyFile)
	assertContains(t, body, "| Command ID | Exit code | Started at | Completed at | Duration ms | Data rows | Data bytes | Data SHA256 | Output summary |")
...
func TestValidateRepoReportsInvalidExecutorDataSHA256(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	evidenceRel := "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/evidence/executor-export-run.json"
	writeFileForTest(t, root, evidenceRel, `{
  ...
  "commands": [
    {
      ...
      "shell_command": "sqlserver2tidb-executor export --execute",
      "exit_code": 0,
      "output": "output sha256: not-a-sha\n",
      "started_at": "2026-01-02T03:04:05Z",
      "completed_at": "2026-01-02T03:04:06Z",
      "duration_ms": 1000,
      "data_rows": 2,
...
func TestGenerateDataMovementPlansRejectsUnsupportedImportEngine(t *testing.T) {
	...
	_, err := GenerateDataMovementPlans(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", DataMovementPlanSpec{
		...
		ChunkSizeRows:   1000000,
		ExportFormat:    "csv",
		ImportEngine:    "bulk-loader",
	})
	if err == nil {
		t.Fatal("GenerateDataMovementPlans() expected unsupported import engine error")
	}
	assertContains(t, err.Error(), "import engine bulk-loader is not supported by sqlserver2tidb-executor")
}

func TestGenerateDataMovementPlansSupportsTiDBLightningEngine(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	...
	exportPlan := readFile(t, root, "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/export-plan.yaml")
	assertContains(t, exportPlan, "null_encoding: backslash-n")
	assertContains(t, exportPlan, "id: app.orders.000001")
	assertContains(t, exportPlan, "output_uri: s3://migration-bucket/sqlserver2tidb/full/app.orders.000001.csv")

	importPlan := readFile(t, root, "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml")
	assertContains(t, importPlan, "engine: tidb-lightning")
	assertContains(t, importPlan, "data_source_uri: s3://migration-bucket/sqlserver2tidb/full")
	assertContains(t, importPlan, "source_uri: s3://migration-bucket/sqlserver2tidb/full/app.orders.000001.csv")
	if strings.Contains(importPlan, "fields:") {
		t.Fatalf("Lightning import plan should rely on CSV headers, not IMPORT INTO fields:\n%s", importPlan)
	}
}
...
func TestGenerateDataMovementPlansRejectsHTTPPrefixForTiDBImportIntoEngine(t *testing.T) {
	...
	assertContains(t, err.Error(), "object URI prefix scheme https is not supported by executable tidb-import-into data plans")
}
...
func TestRunExportWorkerRejectsUnsupportedExportOutputURI(t *testing.T) {
	...
	exportPlan = strings.Replace(exportPlan,
		...
		"ftp://migration-bucket/dbo.orders.000001.csv",
		1,
	)
	...
	assertContains(t, err.Error(), "export chunk dbo.orders.000001 output_uri scheme ftp is not supported by sqlserver2tidb-executor")
}
...
func TestPrepareWorkerExecutorBuildsTiDBLightningImportCommand(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	must(t, GenerateSchemaDraftOnly(root))
	_, err := GenerateDataMovementPlans(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", DataMovementPlanSpec{
		ObjectURIPrefix: "s3://migration-bucket/sqlserver2tidb/full",
		ChunkSizeRows:   3000000,
		ExportFormat:    "csv",
		ImportEngine:    "tidb-lightning",
	})
	if err != nil {
	...
	if spec.Stage != "import" || spec.PayloadHash != hash || len(spec.Commands) != 1 {
		t.Fatalf("executor spec = %+v, want one Lightning import command and hash %s", spec, hash)
	}
	command := spec.Commands[0]
	if command.ID != "tidb-lightning" {
		t.Fatalf("command ID = %q, want tidb-lightning", command.ID)
	}
	assertContains(t, command.ShellCommand, "sqlserver2tidb-executor import")
	assertContains(t, command.ShellCommand, "--job-id tidb-lightning")
	assertContains(t, command.ShellCommand, "--engine tidb-lightning")
	assertContains(t, command.ShellCommand, "--source-uri s3://migration-bucket/sqlserver2tidb/full")
	assertContains(t, command.ShellCommand, "--import-plan clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml")
	if strings.Contains(command.ShellCommand, "--target-object") {
		t.Fatalf("Lightning command should import from the plan, not one target object: %s", command.ShellCommand)
	}
}

func TestPrepareWorkerExecutorRejectsTiDBLightningDataSourceURIMismatch(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	must(t, GenerateSchemaDraftOnly(root))
	_, err := GenerateDataMovementPlans(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", DataMovementPlanSpec{
	...
	importPlanRel := "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml"
	importPlan := readFile(t, root, importPlanRel)
	importPlan = strings.Replace(importPlan, "data_source_uri: s3://migration-bucket/sqlserver2tidb/full", "data_source_uri: s3://migration-bucket/sqlserver2tidb/other", 1)
	writeFileForTest(t, root, importPlanRel, importPlan)
	setReviewPlanStatus(t, root, "import", "reviewed")
	hash, err := ComputePayloadHashForStage(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", "import")
	if err != nil {
	...
	if err == nil {
		t.Fatal("PrepareWorkerExecutor() expected data_source_uri mismatch error")
	}
	assertContains(t, err.Error(), "tidb-lightning data_source_uri")
	assertContains(t, err.Error(), "does not match import job source directory")
}

func TestPrepareWorkerExecutorPassesLightningNullEncodingToExportCommands(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	must(t, GenerateSchemaDraftOnly(root))
	_, err := GenerateDataMovementPlans(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", DataMovementPlanSpec{
	...
	assertContains(t, spec.Commands[0].ShellCommand, "--null-encoding backslash-n")
	assertContains(t, spec.Commands[0].ShellCommand, "--output-uri s3://migration-bucket/sqlserver2tidb/full/app.orders.000001.csv")
}
...
func TestPrepareWorkerExecutorRejectsSQLInsertImportJobFields(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	must(t, GenerateSchemaDraftOnly(root))
	must(t, GenerateDataPlansOnly(root))
	...
	_, err = PrepareWorkerExecutor(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", "import", WorkerExecutorPrepareSpec{})
	if err == nil {
		t.Fatal("PrepareWorkerExecutor() expected sql-insert fields error")
	}
	assertContains(t, err.Error(), "import job import-dbo.orders.000001 fields are only supported with tidb-import-into")
}
...
func TestPrepareWorkerExecutorRejectsUnsupportedImportEngine(t *testing.T) {
	...
	importPlanRel := "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/import-plan.yaml"
	importPlan := readFile(t, root, importPlanRel)
	importPlan = strings.Replace(importPlan, "engine: sql-insert", "engine: bulk-loader", 1)
	writeFileForTest(t, root, importPlanRel, importPlan)
	setReviewPlanStatus(t, root, "import", "reviewed")
	hash, err := ComputePayloadHashForStage(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", "import")
	if err != nil {
...
`)
	...
	_, err = PrepareWorkerExecutor(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", "validation", WorkerExecutorPrepareSpec{})
	if err == nil {
		t.Fatal("PrepareWorkerExecutor() error = nil, want invalid target object error")
	}
	assertContains(t, err.Error(), "validation row_count check orders-row-count target_object must be table or database.table")
}
...
func TestRunImportWorkerPreservesImportJobFields(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	must(t, GenerateSchemaDraftOnly(root))
	_, err := GenerateDataMovementPlans(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a", DataMovementPlanSpec{
...
func TestRunImportWorkerRejectsSQLInsertImportJobFields(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	must(t, GenerateSchemaDraftOnly(root))
	must(t, GenerateDataPlansOnly(root))
	...
	_, err = RunImportWorker(root, "prod-sqlserver-a", "sales-db-to-tidb-prod-a")
	if err == nil {
		t.Fatal("RunImportWorker() expected sql-insert fields error")
	}
	assertContains(t, err.Error(), "import job import-dbo.orders.000001 fields are only supported with tidb-import-into")
}
...
func TestRunValidationWorkerWritesBusinessSQLPlanCheckStatus(t *testing.T) {
	root := t.TempDir()
	createValidationWorkerProject(t, root, dataWorkerInventory())
	must(t, GenerateSchemaDraftOnly(root))
	writeFileForTest(t, root, "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/plan/validation-plan.yaml", `status: reviewed
...
`)
	...
	state := readFile(t, root, "clusters/prod-sqlserver-a/projects/sales-db-to-tidb-prod-a/state/validation-status.yaml")
	assertContains(t, state, "status: passed")
	assertContains(t, state, "name: validation_plan_checks_valid")
	assertContains(t, state, "validation checks are structurally valid")
